...
| 集成式游戏工作区与 Studio 抽屉 | `src/airp/web/index.html`、`src/airp/web/game-workspace-contract.js`、`src/airp/web/game-drawer.js`、`tests/browser/workspace.spec.mjs`、`tests/browser/runtime-monitor.spec.mjs`、`docs/specs/integrated-game-studio-workspace.md` | 游戏页提供互斥的顶部下拉抽屉；Monitor 保留唯一活动 Graph 下拉和 Graph/Trace 上下文；最新回复直接提供 reroll/rollback；Regex 绑定唯一归 Agent 编辑器；四类 Studio 保存携带 `expected_revision`，安全启动诊断不回显 key，运行中保存从下一 Task 生效；Playwright 覆盖桌面/移动布局、溢出与交叠 |
...
| 世界书按需 capability | `src/airp/engine/worldbook_library.py`、`src/airp/host/rp/tools.py`、`tests/test_studio_worldbooks.py`、`tests/test_worldbook_drawer.py` | Project 可绑定多本 Worldbook，Agent 通过 exact-title tool 读取正文；同名 Project 的引用提示包含稳定 ID，重复角色卡导入保持独立 Worldbook 定义 |
...
| Agent Trace/SSE | `src/airp/server.py`、`src/airp/host/rp/trace.py`、`src/airp/host/rp/session_runtime.py`、`tests/browser/runtime-monitor.spec.mjs` | 保留当前/最近 Graph Run；输出节点通过 durable SSE 提供提交前流式预览；真实 Pi 浏览器路径覆盖两回合、五节点/四交接、失败整图重试、Node Detail、空正文、刷新恢复与四种视口 |
