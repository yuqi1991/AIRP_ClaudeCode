...
| Content-neutral Agent Framework direction | Accepted architecture | Framework 只负责 Agent/Graph/Artifact/Trace 运作；当前 RP Turn Adapter 继续负责正则、MVU、回合解析和故事投影，Project 选择 Adapter；生产 engine 已迁至 `src/airp/engine`，`skills/engine` 为兼容 import Adapter | ADR-0018 | handler、静态资源和 CLI 启动脚本仍需继续迁出 `skills/` |
