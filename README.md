...
服务默认只监听 `127.0.0.1:8765`。浏览器打开 `http://127.0.0.1:8765/` 进入 AIRP 游戏工作区；顶部抽屉提供全部 Studio 配置入口，旧的 `/studio` 路径会直接打开同一工作区，不再提供独立 Studio 页面。先配置 Provider/API key，创建或选择 Agent、Graph、Project，再在游戏抽屉中选择当前 Graph。需要局域网访问时显式设置 `AIRP_HOST`，暴露模式会要求 capability token。
