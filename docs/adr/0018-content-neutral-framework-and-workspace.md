...
4. 生产 Python 代码迁移到 `src/airp`，静态资源迁移到 `web`，只读默认资源迁移到 `resources`，用户可变对象统一归属 Workspace。当前 engine 已迁移到 `src/airp/engine`；`skills/engine` 只保留兼容 import Adapter，handler、静态资源和 CLI 入口继续作为过渡层。
...
- 目录迁移会产生一段兼容期；`skills/` 的入口可以暂时保留，但不再是长期生产包名。
