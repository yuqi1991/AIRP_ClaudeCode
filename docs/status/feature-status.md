...
| Atomic Turn Commit | Experimental | 结构化质量门禁、commit 时严格 MVU/schema 校验、同 task 有界修订、optimistic revision、唯一 commit 与可重试且幂等的兼容投影，保证玩家只看到已提交且一致的正式回合 | `engine/runtime.py`、`engine/quality.py`、`engine/mvu.py`、`skills/tests/test_session_turn_runtime.py`、`skills/tests/test_narrative_director.py`、Ticket 05 contract、ADR-0006 | 仍未覆盖 reroll/rollback 分支语义、崩溃恢复分类、真实浏览器/SSE 与真实 DeepSeek E2E；这些留待 Ticket 06/07 与最终选型门槛 |
| Session Event Stream / Command API | Experimental | 玩家命令经幂等 `SessionCommandService` 进入 runtime；stdlib HTTP 暴露 submit/cancel/snapshot 与 SSE（按 sequence 续接 queued→preview→committed→terminal）；刷新后可从同一 DB 恢复 snapshot/commit/projection；不依赖 legacy `.pending`/`input.txt`/`wait_pending` | `engine/commands.py`、`skills/runtime_server.py`、`skills/tests/test_session_event_stream.py`、Ticket 04 contract、ADR-0007 | 单 Session tracer bullet；reroll/rollback 仅预留 `not_implemented`；无 crash lease/recovery（Ticket 07）；SSE 以 FakeProvider/ScriptedDirector 验证，真实浏览器/DeepSeek 仍属最终选型门槛 |
