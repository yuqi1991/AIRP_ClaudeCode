...
| Sequential Writing-Agent Graph | Experimental | 文件型 graph 以稳定 node ID/role/order/model 驱动受限顺序 writing-role pipeline；单节点也产生 node lifecycle events，浏览器 Agent Trace 实时显示节点、模型调用和工具运行；角色/NPC 仍是世界状态实体而非 Agent | `src/airp/engine/agent_graph.py`、`src/airp/engine/runtime_config.py`、`src/airp/web/graphs/default.json`、`skills/tests/test_agent_graph.py` | 明确不支持任意条件 DSL；多节点会线性增加 provider 调用与延迟；当前 Trace 是执行时间线，不是可编辑画布或分支 DAG |
