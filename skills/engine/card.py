"""engine.card — 卡片数据访问 + state.js 存储层（纯 I/O）。

从 handler.py 剪切的存储层函数，保持函数名与签名不变。
handler.py 通过 `from engine.card import ...` 引入同名符号，调用点无需修改。

STYLES 路径：engine/card.py 的 parent.parent 是 skills/，指向 skills/styles/，
与 handler.py 中的 STYLES 常量同路径。
"""
import json
import re
from pathlib import Path

STYLES = Path(__file__).parent.parent / "styles"
