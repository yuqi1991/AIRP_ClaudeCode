...
| 默认协作套件一次性安装 | `src/airp/default_collaboration_suite.py`、`tests/test_default_collaboration_suite_recovery.py`、`tests/test_default_collaboration_suite_concurrency.py` | Workspace 锁、pending WAL、实际 ID receipt、逆序补偿、重启恢复和脱敏启动诊断已覆盖；complete 后不检查、修复或复活用户编辑/删除的对象 |
